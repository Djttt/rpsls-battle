...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans selection:bg-purple-500 selection:text-white pb-20">
      ...
      {/* Header */}
      <header
        className={`bg-slate-900 border-b border-slate-800 p-4 fixed top-0 left-0 right-0 z-50 shadow-2xl transition-transform duration-300 ${isHeaderVisible ? 'translate-y-0' : '-translate-y-full'
          }`}
      >
          ...
          <div className="flex items-center space-x-6 text-sm md:text-base font-mono">
            ...
            {currentUser && (
              <button
                ...
                className="ml-4 p-2 rounded-full hover:bg-slate-800 text-slate-400 hover:text-red-400 transition-colors"
                title="Logout"
              >
                <LogOut size={20} />
              </button>
